  ...
  const server = createServer({
    ...
    routes() {
      ...
      this.get('/schools', (schema: any) => {
        ...
        console.log('📋 GET /api/schools:', schoolsWithClasses.length, 'registros');
      ...
      this.get('/schools/:id', (schema: any, request) => {
        ...
        const classes = schema.all('class').models
          .filter((c: any) => c.attrs.escolaId === id)
          .map((c: any) => c.attrs);
      ...
      this.post('/schools', (schema: any, request) => {
        ...
        if (!dados.nome || dados.nome.trim().length < 3) {
          return new Response(400, {}, { erro: 'Nome é obrigatório (mínimo 3 caracteres)' });
        }
        if (!dados.endereco || dados.endereco.trim().length < 5) {
          return new Response(400, {}, { erro: 'Endereço é obrigatório (mínimo 5 caracteres)' });
        }
      ...
      this.get('/schools/:schoolId/classes', (schema: any, request) => {
        ...
        const classes = schema.all('class').models
          .filter((c: any) => c.attrs.escolaId === schoolId)
          .map((c: any) => c.attrs)
          .sort((a: any, b: any) => a.nome.localeCompare(b.nome, 'pt-BR'));

        console.log('📋 GET /api/schools/' + schoolId + '/classes:', classes.length, 'turmas');
        return classes;
      });
      ...
      this.post('/classes', (schema: any, request) => {
        ...
        if (!dados.nome || dados.nome.trim().length < 2) {
          return new Response(400, {}, { erro: 'Nome é obrigatório (mínimo 2 caracteres)' });
        }
        if (!dados.turno) {
      ...
      this.put('/classes/:id', (schema: any, request) => {
        ...
        if (dados.nome !== undefined && dados.nome.trim().length < 2) {
          return new Response(400, {}, { erro: 'Nome deve ter mínimo 2 caracteres' });
        }
